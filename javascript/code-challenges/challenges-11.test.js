...
function transformToLis(obj) {
  ...
  for (let i = 0; i < Object.keys(obj).length; i++) {
    htmlArr.push(`<li>${Object.keys(obj)[i]}: ${Object.values(obj)[i]}</li>`);
  }
  return htmlArr;
};
    ...
    if (input[i].length === 0) {
      ...
      input[i].forEach(item => {
        if (typeof item === 'number' && item % 5 === 0) {
          filteredArr.push(Math.pow(2, item));
        }
      });
      finalArr.push(filteredArr);
      filteredArr=[];
    }
  }
...
let findShortest = (data) => {
  let shortest = data.reduce((acc, item) => {
    if (item.height > acc) acc = item.height;
    return acc;
  }, 0);
  let arr = data.filter(item => {
    return item.height === shortest;
  });
  return arr[0].name;
};
