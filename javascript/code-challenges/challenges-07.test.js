...
const sortStarWarsCharacters = (starWarsArr) => {
  starWarsArr.sort((a, b) => {
    if (Number(a.height) > parseInt(b.height)) {
      return -1;
...
const removeThree = (idx, arr) => {
  let newArr = [];
  arr.forEach(value => {
    if (value !== arr[idx] && value !== arr[idx + 1] && value !== arr[idx + 2]) {
      newArr.push(value);
    }
  });
  return newArr;
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(ingredient => {
    let str1 = ingredient.slice(ingredient.indexOf(' ') + 1);
    result.push(str1.slice(str1.indexOf(' ') + 1));
  });
  return result;
};
...
const extractVowels = (str) => {
  ...
  extractedVowels = [extractedVowels.join('')];
  console.log(extractedVowels);

  extractedVowels.unshift(arr.join(''));
