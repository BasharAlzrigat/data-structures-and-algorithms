...
const houseSize = (arr) => {
  const sizes = [];
  arr.forEach(item => {
    if (!sizes.includes(item.house)) {
      sizes.push(item.house);
    }
  });
  sizes.forEach((item, index) => {
    sizes[index] = ({ house: item, members: 1 });
  });
  sizes.forEach(item => {
    for (let i = 0; i < arr.length; i++) {
      if (arr[i].house === item.house) {
        if (arr[i].children) {
          item.members += arr[i].children.length;
        }
        if (arr[i].spouse) {
          item.members += 1;
        }
...
const houseSurvivors = (arr) => {
  const survivors = [];
  arr.forEach(item => {
    if (!survivors.includes(item.house)) {
      survivors.push(item.house);
    }
  });
  survivors.forEach((item, index) => {
    survivors[index] = ({ house: item, members: 1 });
  }); for (let i = 0; i < arr.length; i++) {
    survivors.forEach(item => {
      if (arr[i].house === item.house) {
        item.house = arr[i].house;
        if (arr[i].children) {
          item.members += arr[i].children.length;
        }
